...
def load_data():
    ...
    return cust_agg, orders, order_items, products

cust_agg, orders, order_items, products = load_data()
...
orders2 = orders.merge(order_items[["order_id", "price"]], on="order_id", how="left")
monthly_rev = (
...
feature_cols = [
    "frequency", "monetary",
    "avg_order_value", "freight_ratio", "avg_review_score"
]
...
basket = (
    ...
    .pivot_table(index="order_id", columns="product_id", values="value",
...
with tabs[0]:
    ...
    with c1:
        st.plotly_chart(px.histogram(
            cust, x="recency_days", nbins=30,
            title="Recency Distribution"
        ), use_container_width=True)

    with c2:
        st.plotly_chart(px.histogram(
            cust, x="monetary", nbins=30,
...
with tabs[1]:
    ...
    # ------------------------------------------
    # 1. Segment Profile Radar Chart (selectable)
    # ------------------------------------------
    ...
    # ------------------------------------------
    # 3. Key Metrics by Segment (Bar Charts)
    # ------------------------------------------
...
with tabs[2]:
    ...
    with st.expander("🔍 Inspect Individual Customer"):
        cid = st.selectbox("Select Customer ID", cust["customer_id"].head(500))
        doc = cust[cust["customer_id"] == cid].iloc[0]
        st.json({
            "Customer ID": cid,
...
with tabs[3]:
    ...
    cid = st.selectbox("Select Customer", cust["customer_id"].sample(300))

    cdf = cust[cust["customer_id"] == cid].iloc[0]
    ...
    last_orders = orders[orders["customer_id"] == cid]["order_id"]
    last_items = order_items[order_items["order_id"].isin(last_orders)]

    st.dataframe(last_items.tail(5))
    ...
    if subset.empty:
        ...
        st.dataframe(subset.sort_values("lift", ascending=False).head(10))
...
with tabs[4]:
    ...
    st.dataframe(hv[["customer_id", "monetary", "frequency",
                     "recency_days", "churn_prob"]])
